...
async def get_ai_settings(admin: dict = Depends(require_admin)):
    ...
    settings = await db.ai_settings.find({}, {"_id": 0}).to_list(100)
    
    # Get environment keys as fallback
    providers_config = {
        'gemini': {
            'name': 'Google Gemini',
    ...
    for setting in settings:
        ...
        if provider in providers_config:
            providers_config[provider]['keys_count'] = len(setting.get('api_keys', []))
            providers_config[provider]['api_keys'] = setting.get('api_keys', [])
    
    # If no database settings, check environment
    if providers_config['gemini']['keys_count'] == 0:
        gemini_keys = [k for k in GEMINI_API_KEYS if k]
        providers_config['gemini']['keys_count'] = len(gemini_keys)
        providers_config['gemini']['api_keys'] = gemini_keys
    
    if providers_config['azure']['keys_count'] == 0:
        azure_key = os.environ.get('AZURE_OPENAI_KEY')
        if azure_key:
            providers_config['azure']['keys_count'] = 1
            providers_config['azure']['api_keys'] = [azure_key]
...
async def update_ai_settings(keys_update: AIKeysUpdate, admin: dict = Depends(require_admin)):
    ...
    await db.ai_settings.update_one(
        {"provider": keys_update.provider},
        {"$set": {
            "provider": keys_update.provider,
            "api_keys": keys_update.api_keys,
            "updated_at": datetime.now(timezone.utc).isoformat()
        }},
        upsert=True
    )
    ...
    await log_audit(
        db,
        action=f"ai_keys_updated_{keys_update.provider}",
        user_id=admin['id'],
        user_email=admin['email'],
        status="success",
        details={"provider": keys_update.provider, "keys_count": len(keys_update.api_keys)}
    )
    
    return {
        "message": "API keys updated successfully",
        "provider": keys_update.provider,
        "keys_count": len(keys_update.api_keys)
    }
...
async def get_available_providers(user: dict = Depends(get_current_user)):
    ...
    # Check Gemini
    gemini_settings = await db.ai_settings.find_one({"provider": "gemini"})
    if gemini_settings and gemini_settings.get('api_keys'):
        providers.append({
            "id": "gemini",
            "name": "Google Gemini",
            "name_ar": "جوجل جيميناي",
