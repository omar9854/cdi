...
# Find and replace Gemini chat section in chat_with_ai_by_path
old_gemini_section = '''        try:
...
if old_gemini_section in content:
    content = content.replace(old_gemini_section, new_vllm_section)
    print("✅ Replaced Gemini chat with vLLM")
else:
    print("⚠️ Gemini section not found exactly")

# Also replace the retry logic section that follows
old_retry_section = '''            for attempt in range(max_retries):
                try:
...
if old_retry_section in content:
    content = content.replace(old_retry_section, "")
    print("✅ Removed Gemini retry section")
