    ...
    <div className="min-h-screen bg-gradient-to-br from-[#f0f9ff] via-[#e0f2fe] to-[#f0fdfa]">
      ...
      <main className="container mx-auto px-4 py-8 max-w-6xl" data-testid="analysis-page">
        <div className="mb-6 fade-in">
          <Button variant="ghost" onClick={() => navigate('/dashboard')} className="mb-4 text-white hover:bg-white/10" data-testid="back-button">
          ...
          <p className="text-cyan-300">{t('analysisOf')} {t('clinicalNotes')}</p>
        </div>

        {/* Doctor Notes */}
        <Card className="mb-6 fade-in bg-white/10 backdrop-blur-xl border-white/20">
          <CardHeader>
            <CardTitle className="text-2xl text-white">{t('clinicalNotes')}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-4" data-testid="doctor-notes">
              {note?.doctor_notes?.map((dn, idx) => (
                <div key={idx} className="bg-white/5 p-4 rounded-lg border-r-4 border-cyan-500">
                  <p className="font-semibold text-cyan-400 mb-2">
                    {note.doctor_notes[idx].specialty}
                  </p>
                  <pre className="whitespace-pre-wrap text-white/90 font-sans">{dn.text}</pre>
                </div>
              ))}
            </div>
          </CardContent>
        ...
        {!analysis && (
          <Card className="mb-6 fade-in bg-white/10 backdrop-blur-xl border-white/20">
            <CardContent className="text-center py-8">
              ...
              <h3 className="text-2xl font-semibold text-white mb-2">{t('readyToAnalyze')}</h3>
              <p className="text-white/70 mb-4">{t('aiAnalysisDescription')}</p>
        ...
        {analysis && (
          <div className="space-y-6 fade-in">
            ...
            <div className="flex gap-4 justify-end flex-wrap">
              ...
              <Button onClick={() => handleExport('pdf')} variant="outline" className="border-white/30 text-white hover:bg-white/10" data-testid="export-pdf-button">
                {language === 'ar' ? <FileDown className="ml-2" /> : <FileDown className="mr-2" />}
                {t('exportPDF')}
              </Button>
              <Button onClick={() => handleExport('excel')} variant="outline" className="border-white/30 text-white hover:bg-white/10" data-testid="export-excel-button">
                    ...
                    {analysis.diagnoses_to_document.map((diag, idx) => (
                      <div key={idx} className="bg-white/5 p-4 rounded-lg border-r-4 border-cyan-500">
                        <div className="flex justify-between items-start gap-4">
                          ...
                          <span className="bg-cyan-500 text-white px-3 py-1 rounded-full text-sm font-semibold whitespace-nowrap">
                            {diag.icd_code}
                          </span>
                        </div>
                      </div>
                ...
                <CardContent>
                  <div className="space-y-3">
                    {analysis.secondary_diagnoses.map((diag, idx) => (
                      <div key={idx} className="bg-white/5 p-4 rounded-lg border-r-4 border-purple-500">
                        <div className="flex justify-between items-start gap-4">
                          <div className="flex-1">
                            <p className="text-white font-medium mb-1">
                              {language === 'ar' ? diag.diagnosis_ar : diag.diagnosis_en}
                          ...
                          <span className="bg-purple-500 text-white px-3 py-1 rounded-full text-sm font-semibold whitespace-nowrap">
                            {diag.icd_code}
                          </span>
                        </div>
                      </div>
                ...
                <CardContent>
                  <div className="space-y-3" data-testid="missing-documentation">
                    {analysis.missing_documentation.map((item, idx) => (
                      <div key={idx} className="bg-white/5 p-4 rounded-lg border-r-4 border-orange-500">
                ...
                <CardContent>
                  <div className="space-y-3" data-testid="gaps">
                    {(language === 'ar' ? analysis.gaps_ar : analysis.gaps_en).map((gap, idx) => (
                      <div key={idx} className="bg-white/5 p-4 rounded-lg border-r-4 border-amber-500">
                ...
                <CardContent>
                  <div className="space-y-3" data-testid="queries">
                    {(language === 'ar' ? analysis.queries_ar : analysis.queries_en).map((query, idx) => (
                      <div key={idx} className="bg-white/5 p-4 rounded-lg border-r-4 border-green-500">
            ...
            {(analysis.summary_ar || analysis.summary_en) && (
              <Card className="bg-white/10 backdrop-blur-xl border-white/20">
                <CardHeader>
                  <CardTitle className="text-2xl text-white">{t('comprehensiveSummary')}</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="bg-white/5 p-6 rounded-lg" data-testid="full-analysis">
