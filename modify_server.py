...
# 1. Add local_llm import if not exists
import_line = "from local_llm import analyze_clinical_notes, get_model, initialize_drg_lookup"
if "from local_llm import" not in content:
    # Add after aiosmtplib import
    content = content.replace(
        "import aiosmtplib",
        "import aiosmtplib\n" + import_line
    )
    print("✅ Added local_llm import")

# 2. Make Gemini optional
old_gemini_check = '''if not GEMINI_API_KEYS:
    raise ValueError("No Gemini API keys found in environment variables")

print(f"✅ Loaded {len(GEMINI_API_KEYS)} Gemini API keys for rotation")'''

new_gemini_check = '''# In offline mode, Gemini is optional
if GEMINI_API_KEYS:
    print(f"✅ Loaded {len(GEMINI_API_KEYS)} Gemini API keys for rotation")
else:
    print("⚠️ No Gemini API keys - Running in LOCAL AI ONLY mode")'''

if old_gemini_check in content:
    content = content.replace(old_gemini_check, new_gemini_check)
    print("✅ Made Gemini optional")

# 3. Replace analyze_with_ai function
new_analyze_function = r'''async def analyze_with_ai(notes_text: str, doctor_notes: List[Dict], provider: str = 'local') -> Dict:
    """
...
# Pattern: from "async def analyze_with_ai" to the next top-level definition
pattern = r"async def analyze_with_ai\(notes_text: str, doctor_notes: List\[Dict\], provider: str = 'phi3'\) -> Dict:.*?(?=\n# Health check route)"
content = re.sub(pattern, new_analyze_function, content, flags=re.DOTALL)
print("✅ Replaced analyze_with_ai function")
